/*
  # Add shared vision boards functionality
  ...
  1. New Tables
    - `shared_vision_boards`
      ...
      - `board_data` (jsonb) - complete board state including items and layout
      - `total_budget` (numeric) - total budget of items on board
      - `expires_at` (timestamp, optional) - when the share link expires
      - `view_count` (integer) - number of times the board has been viewed
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on shared_vision_boards table
    - Add policies for authenticated users to manage their own shared boards
    - Add policy for anonymous users to view shared boards via token
*/
...
-- Authenticated users can manage their own shared boards
...
-- Anonymous users can view shared boards via token (if not expired)
